...
odoo.define('stock_3d_view.action_open_form_3d_view', [
    ...
    var open_form_3d_view = AbstractAction.extend({
        ...
        Open3DView: function () {
		   ...
		   var dialogs = null; 
		   var wh_id; 
		   var location_id = self.searchModel.config.context.loc_id || localStorage.getItem("location_id"); //sets location_id and company_id in local storage if (self.searchModel.config.context.loc_id != null) { localStorage.setItem("location_id", self.searchModel.config.context.loc_id); localStorage.setItem("company_id", self.searchModel.config.context.company_id);
		   if (self.searchModel.config.context.loc_id != null) { 
			...
			var color1 = document.createElement("div"); color1.classList.add("square1");
			...
			colorDiv.appendChild(colorText4); start(); 
			/** 
			...
			*/ 
			async function start() { 
				...
				const baseMaterial = new THREE.MeshBasicMaterial({ color: 0xffffff, transparent: false, opacity: 1, side: THREE.FrontSide, }); 
				...
				for (let [key, value] of Object.entries(data)) { 
					...
					if ((value[0] > 0) || (value[1] > 0) || (value[2] > 0) || (value[3] > 0) || (value[4] > 0) || (value[5] > 0)) { 
						...
						if (localStorage.getItem("location_id") == value[6]) { 
							if (loc_quant[0] > 0) { 
								if (loc_quant[1] > 100) { 
									loc_color = 0xcc0000; loc_opacity = 0.8; 
								} else if (loc_quant[1] > 50) { 
									loc_color = 0xe6b800; loc_opacity = 0.8; 
								} else { 
									loc_color = 0x00802b; loc_opacity = 0.8; 
								} 
							} else { 
								if (loc_quant[1] == -1) { 
									loc_color = 0x00802b; loc_opacity = 0.8; 
								} else { 
									loc_color = 0x0066ff; loc_opacity = 0.8; 
								} 
							} 
						} else { 
						...
						loader.load('https://threejs.org/examples/fonts/droid/droid_sans_bold.typeface.json', function(font) { 
							const textcolor = 0x000000; const textMat = new THREE.MeshBasicMaterial({ 
								color: textcolor, 
								side: THREE.DoubleSide, 
							}); 
							const textmessage = key; if (value[3] > value[4]) { 
